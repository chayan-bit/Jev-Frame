# Jev-Frame architecture

This document describes the design, public contracts, and architecture of Jev-Frame in depth.
...
Only `PURE` and `READ` tools are admitted on the read-only path, tool outputs are strictly validated, and declared blocking callables use bounded worker threads whose underlying work may outlive cancellation of the await.
...
The runtime additionally admits declared `MUTATION` tools only after a versioned semantic policy, any configured required checkpoint, a current exact-action authorization, source and argument revalidation, write admission, and durable intent recording.
...
Framework integration is a first-class part of the product.
